...
table = PrettyTable(["Thread", "Total Success", "Total Failures", "Average Time", "Running Query Time"])
for i in data.keys():
    if i.startswith("Thread"):
        running_query_time = ""
        if data[i]["currently_running_query"]:
            running_query_time = data[i]["currently_running_query_time"]
        table.add_row([i, data[i]["successful"], data[i]["failures"], data[i]["average_query_time"], running_query_time])
print(table)
